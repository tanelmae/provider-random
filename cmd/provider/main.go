...
import (
	...
	"github.com/community/provider-random/apis"
	"github.com/community/provider-random/apis/v1alpha1"
	"github.com/community/provider-random/config"
	"github.com/community/provider-random/internal/clients"
	"github.com/community/provider-random/internal/controller"
	"github.com/community/provider-random/internal/features"
)

func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Random").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-random"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-random",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add Random APIs to scheme")
	...
	kingpin.FatalIfError(controller.Setup(mgr, o), "Cannot setup Random controllers")
